...
# Placeholder for intentional vulnerability: Hardcoded AES key for file transfers (Backdoor 1)
...
# Encrypt file transfer with a hardcoded AES key (Backdoor 1)
def encrypt_file_with_backdoor(file_data):
    aes_key = BACKDOOR_AES_KEY
...
# Handle incoming WebSocket connections (Backdoor 3: Skipping counter validation)
async def handle_client(websocket, path):
    try:
        async for message in websocket:
            message_data = json.loads(message)

            # Skip counter validation for replay attacks (Backdoor 3)
